 ...
 * -------------------------------------------------------------
 */
hdr_void bErrorPrint(hdr_char* fmt, ...);
hdr_void displayBuffer(BufferReader* ptr_Buffer);
hdr_long getFileSize(hdr_char* fname);
hdr_int isNumber(const hdr_char* ns);
hdr_void startReader(hdr_char*, hdr_char*, hdr_char, hdr_int, hdr_int);
...
************************************************************
*/
hdr_void startReader(hdr_char* program, hdr_char* input, hdr_char mode, hdr_int size, hdr_int increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	hdr_int loadSize = 0;		/* the size of the file loaded in the buffer */
	hdr_char symbol;			/* symbol read from input file */
